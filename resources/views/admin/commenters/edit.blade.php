<x-dashboard.main-layout>
    ...
    <div class="card-body">
        ...
        <h1 class="mb-3 text-gray-800 h3">{{ __('Edit Commenter') }}</h1>

        <form class="my-3" action="{{ route('admins.commenters.update', $user->id) }}" method="post"
            ...
            <div class="form-group">
                ...
                @error('password')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                <small class="form-text text-muted">{{ __('Leave blank if you do not want to change the password') }}</small>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="commenter_id" id="commenter_id" 
                    ...
                    @foreach($listings as $listing)
                        <option value="{{ $listing->id }}" @selected((old('commenter_id') ?? $user->commenter_id) == $listing->id)>
            ...
            <button type="submit" class="btn btn-success btn-block mb_40">{{ __('Update Commenter') }}</button>
