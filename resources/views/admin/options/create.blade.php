<x-dashboard.main-layout>
    ...
    <div class="card-body">
        <form class="my-3" action="{{ route('admins.options.store') }}" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <textarea name="description_ar" class="form-control" id="description_ar" placeholder="{{ __('Description in Arabic') }}">{{ old('description_ar') }}</textarea>
            </div>

            <div class="form-group">
                <label for="description_en">{{ __('Description In English') }}</label>
                <textarea name="description_en" class="form-control" id="description_en" placeholder="{{ __('Description in English') }}">{{ old('description_en') }}</textarea>
            </div>
