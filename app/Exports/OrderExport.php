...
class OrderExport implements FromCollection, WithHeadings, WithStyles, WithTitle, ShouldAutoSize, WithEvents
{
    public function __construct(public $orders)
    {
        $this->orders = $orders;
        $this->orders->load(['user','serviceProvider.category','serviceProviderBranch','cashier']);
    }
    public function collection(): Collection
    {
        ...
        return $this->orders->map(function ($order) use ($locale) {
            return [
                optional($order->user)->name ?? __('User Deleted'),
                "\t" . (optional($order->user)->phone ?? __('Phone Deleted')),
                $order->serviceProvider->getTranslation('name',$locale) ?? __('Category Deleted'),
